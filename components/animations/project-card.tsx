  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className={cn("relative z-50 w-[19rem] sm:w-[20rem]", className)}>
            {children}
          </div>
        </div>
      </div>
    ...
    <motion.div className="pointer-events-none w-96 h-80 flex items-center justify-center opacity-0 group-hover/pin:opacity-100 z-[60] transition duration-500">
      <div className="w-full h-full -mt-7 flex-none inset-0">
        <div className="absolute top-0 inset-x-0 flex justify-center">
          <p className="relative flex space-x-2 items-center z-10 rounded-full bg-zinc-950 py-0.5 px-4 ring-1 ring-white/10">
            <span className="relative z-20 text-white text-xs font-bold inline-block py-0.5">
              {title}
            </span>
            <span className="absolute -bottom-0 left-[1.125rem] h-px w-[calc(100%-2.25rem)] bg-gradient-to-r from-red-400/0 via-red-500 to-red-400/0 transition-opacity duration-500 group-hover/btn:opacity-40" />
          </p>
        </div>
        ...
        >
          {[0, 2, 4].map((delay, i) => (
            <motion.div
              ...
              className={`absolute left-1/2 top-1/2 h-[11.25rem] w-[11.25rem] rounded-[50%] bg-red-500/${(i + 1) * 20} shadow-[0_8px_16px_rgb(0_0_0/0.4)]`}
            />
          ))}
        </div>
